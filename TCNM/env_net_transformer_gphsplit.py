...
"""
TCNM/env_net_transformer_gphsplit.py  ── v9-fixed
=============================================
...
# ── Normalization stats for Data3d scalar features ───────────────────────────
# Computed from aligned_data.csv (physically valid range only)
_GPH500_MEAN  = 5843.14
_GPH500_STD   = 50.55
_UV500_SCALE  = 10.0  # divide by 10 m/s, then clip to [-1, 1]
...
def build_env_features_one_step(
    ...
    # ── Data3d scalar features ────────────────────────────────────────────
    # GPH500: z-score with global stats from aligned_data.csv
    for k in ["gph500_mean", "gph500_center"]:
        if isinstance(env_npy, dict) and k in env_npy:
            raw = float(env_npy[k])
            val = 0.0 if (raw == -1 or math.isnan(raw)) else \
                  (raw - _GPH500_MEAN) / (_GPH500_STD + 1e-8)
            val = max(-5.0, min(5.0, val))  # soft clip
        else:
            val = 0.0
        feat[k] = [val]

    # U/V 500hPa: divide by 10 m/s, clip to [-1, 1]
...
class Env_net(nn.Module):
    """
    Environment-Time Network (Eq. 10-13 in paper).
    ...
    [Eq.13] TransformerEncoder over T → e_Env_time [B, d_model]

    Returns (ctx [B, d_model], 0, 0) for API compatibility.
    """
    ...
    def __init__(self, obs_len: int = 8, embed_dim: int = 16, d_model: int = 64):
        ...
        # Eq.11 — 1D-CNN on Data3d scalars (6-dim, treat T as sequence length)
        ...
        # Eq.13 — TransformerEncoder over time dimension
